...
template <typename A, typename BI> BI destroy(A &a, BI b, BI e) {
  while (b != e) {
    --e;
    a.destroy(&*e);
  }
...
public:
  ...
  public:
    ...
    friend bool operator==(const iterator &lhs, const iterator &rhs) {
      ...
      return (lhs.index == rhs.index) && (lhs.outer_index == rhs.outer_index) &&
             (lhs.x == rhs.x);
    }
     ...
     * @return bool
     *
     * output the whether the value of lhs is not equal to rhs (true for not
     *equal)
     */
    friend bool operator!=(const iterator &lhs, const iterator &rhs) {
      return !(lhs == rhs);
  ...
  public:
     ...
     * @param out a size_type index represents the outer index that this
     * iterator is point at
     * output an iterator
     */
...
public:
  ...
  public:
     ...
     */
    const_iterator(const my_deque *q, size_type s = 0)
        : i(const_cast<my_deque *>(q), s) {
      // <your code>
      assert(valid());
    }
...
public:
   ...
   */
  explicit my_deque(size_type s = 0, const_reference v = value_type(),
    ...
    if (s == 0) {
      ...
      size_type temp =
          (_size % INNER_SIZE == 0) ? INNER_SIZE : _size % INNER_SIZE;
  ...
  my_deque(const my_deque &that)
      : _a(that._a), _size(that._size), outer_size(that.outer_size),
   ...
   */
  const_reference back() const { return const_cast<my_deque *>(this)->back(); }
   ...
   * @param v is a const_reference which is used to fill new positions if size
   * is greater than current size
   */
